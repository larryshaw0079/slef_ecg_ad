...
def prepare_data(data_path, train_val_test_split=(5, 2, 3), val_anomaly_ratio=0.5, anomaly_injection_ratio=0.0, verbose=True):
    ...
    if anomaly_injection_ratio > 0.0:
        ...
        selected_indices = np.random.choice(anomaly_indices, size=int(train_samples.shape[0] * anomaly_injection_ratio), replace=False)
    ...
    val_samples = np.concatenate((normal_samples[normal_val_indices], anomalous_samples[:int(anomalous_samples.shape[0]*val_anomaly_ratio)]))
    val_labels = np.concatenate((np.zeros(normal_samples[normal_val_indices].shape[0]), np.ones(anomalous_samples[:int(anomalous_samples.shape[0]*val_anomaly_ratio)].shape[0])))
    val_indices = np.arange(val_samples.shape[0])
    np.random.shuffle(val_indices)
    val_samples = val_samples[val_indices]
    val_labels = val_labels[val_indices]

    test_samples = np.concatenate((normal_samples[normal_test_indices], anomalous_samples[int(anomalous_samples.shape[0]*val_anomaly_ratio):]))
    test_labels = np.concatenate((np.zeros(normal_samples[normal_test_indices].shape[0]), np.ones(anomalous_samples[int(anomalous_samples.shape[0]*val_anomaly_ratio):].shape[0])))
